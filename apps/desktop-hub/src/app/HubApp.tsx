...
const categories: { id: ModuleCategory; label: string; icon: typeof BookOpen }[] = [
    ...
    <main className="main">
      {view === 'welcome' ? <section className="welcome fade-in">
        ...
        <header className="explore-head">
          <div className="explore-heading"><IconButton className="back-button" title="\u8fd4\u56de\u9996\u9875" onClick={() => setView('welcome')}><ArrowLeft size={16} /></IconButton><div className="explore-title"><div className="eyebrow">DAIDAI HUB / SPACES</div><h2>{'\u5de5\u5177\u7a7a\u95f4'}</h2></div></div>
        ...
        <div className="explore-workspace">
          ...
          <section className="explore-results" aria-label={activeCategory.label}>
            {modules.length > 0 ? <div className="module-grid">{modules.map((item) => { const Icon = item.icon; const ready = item.status === 'ready'; return <Button key={item.id} className={`module-card ${!ready ? 'is-disabled' : ''}`} disabled={!ready} onClick={() => ready && setActiveModule(item)}><span className={`module-icon ${item.color}`}><Icon size={22} /></span><span className="module-copy"><strong>{item.title}</strong><small>{item.description}</small></span><ChevronRight className="module-arrow" size={18} />{ready ? <Badge tone="success">{'\u53ef\u7528'}</Badge> : <Badge>{'\u5373\u5c06\u52a0\u5165'}</Badge>}</Button>; })}</div> : <div className="module-empty">{'\u6ca1\u6709\u5339\u914d\u7684\u7a7a\u95f4'}</div>}
          </section>
        </div>
      </section>}
    </main>
